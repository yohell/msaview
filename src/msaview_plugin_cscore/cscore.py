...
def import_extension():
    ...
    package =  '_cscore_%s_%s'
    package %= (''.join(platform.python_version_tuple()[:-1]), platform.machine())
    try:
        tmp = __import__(package +'._cscore', globals(), locals(), [], -1)
        extension_module = tmp._cscore
        del package, tmp
    except Exception, e:
        extension_module = None 
    sys.path.pop(0)
    return extension_module
_cscore = import_extension()
#import _moo as _cscore
